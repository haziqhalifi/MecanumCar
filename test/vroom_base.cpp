 ...
 *
 *  MOTION IMPLEMENTATION: the motion layer writes the STC8's 8 per-wheel PWM
 *    channels directly (via mecanumCar.Motor_*(state, pwm), 0..255) for true
 *    constant-speed control. Corrections are mecanum strafes (heading held).
 *    Still on the table (motion layer only — nothing above it changes):
 *    pivoting about the FRONT SENSOR instead of the chassis center, which
 *    would eliminate ADVANCE_BEFORE_PIVOT_MS entirely.
 *
...
#include <IRremote.h>        // IR remote decoder (uses the 2.x results.value API)
...
// After braking, the turn creeps straight (follow speed) to re-center on the
// new perpendicular line, finishing on white-black-white. If that signature
// isn't reached within this window the turn still completes (the following
// followLine finishes the centering) but prints a one-line notice.
const unsigned long RECAPTURE_TIMEOUT_MS = 500;  // TUNE ON HARDWARE
...
// ---- Speed (real per-wheel PWM, 0..255) -------------------------------------
// The motion layer drives every wheel with an explicit PWM value via
// mecanumCar.Motor_*(state, pwm). This REPLACES the old software duty-cycle
// throttle, which pulsed the motors on/off and let the chassis COAST between
// pulses — the main source of run-to-run inconsistency (variable speed made
// the timed turn constants land a different distance every run, and sensor
// sampling wasn't synced to the pulses). Speed is now constant.

// Straight-line following speed. Lower = slower + more accurate tracking.
const uint8_t FORWARD_SPEED = 60;   // TUNE ON HARDWARE (0..255)

// Diagonal strafe-correction speed (the driven wheels of a forward-left /
// forward-right slide). Keep <= FORWARD_SPEED so a correction stays gentle.
const uint8_t NUDGE_SPEED   = 60;   // TUNE ON HARDWARE (0..255)

// Per-wheel speed for the center-point spin (left pair vs right pair driven in
// opposite directions, rotating about the chassis center). Kept LOW on purpose:
// a slow spin catches the new line precisely and overshoots less, so the
// fixed-time brake pulse matters far less. Raise it if the chassis stalls.
const uint8_t TURN_MOTOR_SPEED = 45;  // TUNE ON HARDWARE (0..255)
 ...
 *  this layer maps intent to explicit per-wheel PWM writes at a constant
 *  commanded speed. motionService() must run every loop().
 *
 *  Corrections are mecanum STRAFES, not rotations: nudgeLeft/Right slide the
 *  chassis diagonally forward-left / forward-right to re-center on the line
 *  WITHOUT changing heading — so a follow leg can no longer slowly rotate
 *  itself off the line. Only the spin (turn primitive) changes heading.
 * ==========================================================================*/
...
static void motionApply(MotionCmd cmd) {
  // Center-point spin (M_SPIN_*/M_BRAKE_*): the left pair and the right pair are
  // driven in OPPOSITE directions at TURN_MOTOR_SPEED, so the chassis rotates
  // about its own center. Motor_*(state, speed): state 1 = forward, 0 = backward
  // (verified against mecanumCar_v2: Advance() drives all four with state 1).
  //   Right turn (CW):  left side FORWARD, right side BACKWARD.
  //   Left  turn (CCW): left side BACKWARD, right side FORWARD.
  // WIRING NOTE: motor polarity varies. If a commanded RIGHT turn physically
  // rotates LEFT, invert all four state bits in BOTH spin patterns below.
  // Nudges are mecanum diagonals (two wheels drive, heading held) — see below.
  switch (cmd) {
    case M_FORWARD:                           // all four forward at FORWARD_SPEED
      mecanumCar.Motor_Upper_L(1, FORWARD_SPEED);
      mecanumCar.Motor_Lower_L(1, FORWARD_SPEED);
      mecanumCar.Motor_Upper_R(1, FORWARD_SPEED);
      mecanumCar.Motor_Lower_R(1, FORWARD_SPEED);
      break;
    // Diagonal strafe correction (forward + lateral, HEADING UNCHANGED). Mirrors
    // the library's LU_Move / RU_Move but at a controlled NUDGE_SPEED.
    case M_NUDGE_L:                           // slide forward-LEFT (line under L)
      mecanumCar.Motor_Upper_L(1, 0);
      mecanumCar.Motor_Lower_L(1, NUDGE_SPEED);
      mecanumCar.Motor_Upper_R(1, NUDGE_SPEED);
      mecanumCar.Motor_Lower_R(1, 0);
      break;
    case M_NUDGE_R:                           // slide forward-RIGHT (line under R)
      mecanumCar.Motor_Upper_L(1, NUDGE_SPEED);
      mecanumCar.Motor_Lower_L(1, 0);
      mecanumCar.Motor_Upper_R(1, 0);
      mecanumCar.Motor_Lower_R(1, NUDGE_SPEED);
      break;
    case M_SPIN_R: case M_BRAKE_R:            // right turn: left fwd, right back
      mecanumCar.Motor_Upper_L(1, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Lower_L(1, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Upper_R(0, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Lower_R(0, TURN_MOTOR_SPEED);
      break;
    case M_SPIN_L: case M_BRAKE_L:            // left turn: left back, right fwd
      mecanumCar.Motor_Upper_L(0, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Lower_L(0, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Upper_R(1, TURN_MOTOR_SPEED);
      mecanumCar.Motor_Lower_R(1, TURN_MOTOR_SPEED);
      break;
    case M_STOP: default:          mecanumCar.Stop();       break;
  }
}

// Runs every loop(): pushes the commanded motion to the STC8 only when it
// CHANGES (the bit-banged I2C bus is slow, so we avoid re-sending every loop).
// Speed is baked into each per-wheel PWM in motionApply(), so there is no
// duty-cycle gating anymore — the wheels hold a constant commanded speed.
void motionService() {
  if (g_motionCmd != g_motionApplied) {
    motionApply(g_motionCmd);
    g_motionApplied = g_motionCmd;
  }
}
...
enum TurnPhase { TP_ADVANCE, TP_BLANK, TP_SEEK, TP_BRAKE, TP_RECAPTURE };
  ...
  switch (g_turn.phase) {
    ...
    case TP_BRAKE:
      if (now - g_turn.phaseStartMs >= TURN_BRAKE_MS) {
        driveStraight();   // creep forward (follow speed) to re-center the array
        g_turn.phase        = TP_RECAPTURE;
        g_turn.phaseStartMs = now;
      }
      break;
    ...
    case TP_RECAPTURE: {
      LineReading s = readLineSensors();
      bool centered = s.middle && !s.left && !s.right;  // white-black-white
      if (centered || now - g_turn.phaseStartMs >= RECAPTURE_TIMEOUT_MS) {
        if (!centered) Serial.println(F("[turn] recapture timeout"));
        stopMotors();
...
static void missionServiceIr() {
  ...
  irrecv.resume();  // re-arm the receiver for the next frame
}
...
void setup() {
  ...
  irrecv.enableIRIn();  // start the IR receiver on A3 (mission triggers)
...
void loop() {
  ...
  motionService();       // apply the commanded motion (constant per-wheel PWM)
}
